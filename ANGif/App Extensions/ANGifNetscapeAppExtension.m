...
- (id)initWithRepeatCount:(UInt16)repeats {
	if ((self = [super init])) {
		...
		applicationData = [NSData dataWithData:appData];
		// app ID and signature
		applicationIdentifier = [NSData dataWithBytes:"\x4E\x45\x54\x53\x43\x41\x50\x45" length:8];
		applicationAuthCode = [NSData dataWithBytes:"\x32\x2E\x30" length:3];
	}
	return self;
}
